...
import { ChatBubbleOvalLeftEllipsisIcon, ClockIcon, FilmIcon, HandThumbUpIcon } from '@heroicons/react/24/outline'
import { t } from '../locale'

export default async function Card({ video }: { video: BilibiliVideo }) {
  return <article className="flex flex-col items-start justify-between">
    ...
    <div className="w-full">
      ...
      <div className="flex justify-between w-full">
        <div className="relative group mt-4 flex items-center gap-x-4">
          <img referrerPolicy="no-referrer" alt="" src={video.owner.face}
               className="h-10 w-10 rounded-full bg-gray-100 ring-1 ring-inset ring-gray-900"/>
          <div className="text-sm leading-6">
            <Link href={`https://space.bilibili.com/${video.owner.mid}`} target="_blank"
                  referrerPolicy="no-referrer"
        ...
        <div className="flex items-end justify-end space-x-1">
          <form action={() => {
            ...
            <button className="text-xs opacity-50 text-gray-500 hover:text-red-500">{t('Blacklisted author')}</button>
          </form>
          <form action={() => {
            'use server'
            addBlackListVideo(video.id)
          }}>
            <button className="text-xs opacity-50 text-gray-500 hover:text-red-500">{t('Blacklisted video')}</button>
          </form>
        </div>
      </div>
    </div>
